...
class BaseScene(Composer):
    ...
    def get_state(self, ctx: Context) -> Dict[str, Any]:
        """Get scene state."""
        if (hasattr(ctx, 'session') and '__scene' in ctx.session and 
            ctx.session['__scene']['id'] == self.scene_id):
    ...
    def set_state(self, ctx: Context, state: Dict[str, Any]) -> None:
        ...
        if '__scene' not in ctx.session or ctx.session['__scene']['id'] != self.scene_id:
            ctx.session['__scene'] = {
                'id': self.scene_id,
                'state': {},
                'entered_at': asyncio.get_event_loop().time()
